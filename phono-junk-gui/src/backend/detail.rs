...
use phono_junk_lib::{PhonoContext, cache_asset_bytes};
...
/// placeholder.
///
/// The worker opens its own `Connection` (matching the
/// `scan`/`identify`/`verify`/`export` pattern) so [`cache_asset_bytes`]
/// can persist the downloaded path back to the `assets` row — that's what
/// makes subsequent views skip the network.
///
...
pub fn spawn_cover_fetch(
    ...
    thread::spawn(move || {
        let conn = match open_database(&db_path) {
            Ok(c) => c,
            Err(e) => {
                let error = format!("open DB for cover fetch: {e}");
                log::warn!("detail: {error}");
                let _ = tx.send(AppMessage::DetailArtFailed { key, error });
                return;
            }
        };
        match cache_asset_bytes(&ctx, &conn, &asset, &cache_dir) {
            Ok(bytes) => {
